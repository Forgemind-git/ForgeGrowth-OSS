...
-- forgecrm_users is created at runtime by the backend's auth.js ensureTables()
-- (no earlier migration creates it), so applying this file standalone — e.g. the
-- CI migration-lint job against a fresh DB — would fail at the ALTERs below with
-- "relation does not exist". Recreate the pre-031 base shape idempotently so the
-- ALTERs always have a table to widen. In production this is a no-op (the table
-- already exists), keeping the migration self-sufficient. Mirrors auth.js.

...
// Which send origins may fire an OUTBOUND keyword trigger. Deliberately only
// human chat replies and broadcasts — automation/agent sends are excluded so an
// automation's own outbound message can never re-trigger it (loop-safe default).
const OUTBOUND_TRIGGER_ORIGINS = new Set(['chat_reply', 'broadcast_log']);
...
async function processJob(job) {
  ...
  // Outbound keyword-trigger evaluation for messages sent THROUGH ForgeGrowth.
  // Meta also echoes every business-sent message back to our webhook, so we key
  // on the wamid to fire each outbound message's trigger exactly once across
  // both paths (see automationEngine dedup).
  //  - Human chat reply / broadcast  → fire now (and claim the wamid so the echo
  //    doesn't double-fire it).
  //  - Automation / agent / anything else → CLAIM the wamid WITHOUT firing, so
  //    its echo can never re-trigger the automation that sent it (loop guard).
  if (wamid && kind !== 'reaction') {
    try {
      const { evaluateOutboundTriggers, markOutboundHandled } = require('../engine/automationEngine');
      if (OUTBOUND_TRIGGER_ORIGINS.has(originRef?.kind)) {
        const { rows } = await pool.query(
          `SELECT message_id, wa_number, contact_number, message_body, message_type, timestamp, phone_number_id
             FROM coexistence.chat_history WHERE message_id = $1`,
          [wamid]
        );
        if (rows[0] && rows[0].message_body) {
          await evaluateOutboundTriggers({ ...rows[0], status: 'sent' });
        }
      } else {
        markOutboundHandled(wamid); // claim so the echo won't re-fire an automation/agent send
      }
    } catch (err) {
      console.error('[sendQueue] outbound trigger eval failed:', err.message);
    }
  }

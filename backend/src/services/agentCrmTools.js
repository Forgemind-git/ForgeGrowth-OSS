...
// native CRM (name, tags, custom fields) — not just an external
// sheet. They're only built when the agent has `crm_tools_enabled` AND there's
// a real live contact (skipped in the test preview). Each executor is scoped to
// a single (wa_number, contact_number) so the LLM can't touch other contacts.
...
function buildCrmTools({ waNumber, contactNumber }) {
  ...
  tools.push({
    ...
    description: 'Add an existing CRM tag to this contact (label leads, e.g. "Hot Lead", "Trial Booked"). The tag must already exist in the CRM; if it doesn\'t this returns an error and you should just carry on.',

...
async function downloadOne(messageId) {
  ...
  try {
    ...
    // Media lives on the WABA that owns the business number, so it can only be
    // read with that account's token — not a process-wide one.
    const account = await getAccountByPhoneNumber(row.wa_number);
    const token = account?.accessToken || null;

    const info = await getMediaInfo(mediaId, token);
    const bin = await downloadMediaBinary(info.url, token);

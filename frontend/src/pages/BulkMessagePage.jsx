...
function StatusBadge({ status }) {
  const config = {
    ...
    SCHEDULED: { bg: '#ede9fe', color: '#5b21b6', border: '#ddd6fe', dot: '#7c3aed' },
            ...
            <div style={{
              ...
              <div style={{ padding: '20px 24px', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>Select Recipients</div>
              ...
              <div style={{
                ...
                {/* Schedule: pick a future local time; the backend fires it on a
                    one-minute tick. Sends nothing now. */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <input
                    type="datetime-local"
                    value={newBroadcastScheduleAt}
                    onChange={e => setNewBroadcastScheduleAt(e.target.value)}
                    title="Schedule this broadcast for a future date and time"
                    style={{
                      padding: '9px 10px', borderRadius: 8, border: `1px solid ${C.border}`,
                      background: 'var(--c-cardBg)', color: C.text, fontSize: 12.5,
                      fontFamily: FONT, outline: 'none',
                    }}
                  />
                  <button
                    onClick={() => handleNewBroadcastSave('SCHEDULED')}
                    disabled={isBroadcastFormInvalid() || !newBroadcastScheduleAt}
                    style={{
                      padding: '10px 16px', borderRadius: 8, border: `1.5px solid ${C.purple}`,
                      background: 'var(--c-cardBg)', color: C.purple,
                      cursor: (isBroadcastFormInvalid() || !newBroadcastScheduleAt) ? 'not-allowed' : 'pointer',
                      fontSize: 13, fontWeight: 700, fontFamily: FONT,
                      opacity: (isBroadcastFormInvalid() || !newBroadcastScheduleAt) ? 0.5 : 1,
            ...
            <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-cardBg)', borderRadius: 14, width: 420, boxShadow: C.shadowLg, padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  style={{ padding: '10px 18px', borderRadius: 8, border: 'none', background: C.primary, color: '#fff', cursor: newBroadcasting ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 700, fontFamily: FONT, display: 'flex', alignItems: 'center', gap: 6 }}
                >

        ...
        <div style={{ flex: 1, minHeight: 0, position: 'relative', borderRadius: 46.5, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#075E54' }}>
          ...
          <div style={{ background: '#075E54', paddingTop: 50, paddingBottom: 8, paddingLeft: 12, paddingRight: 12, color: '#fff', fontFamily: "-apple-system, 'SF Pro Display', system-ui, sans-serif", flexShrink: 0, position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{headerTitle}</div>
                <div style={{ fontSize: 12, opacity: .82, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{headerSubtitle}</div>
              </div>
              {/* Video */}
          ...
          {inputBar ? (
            ...
            <div style={{ background: 'var(--c-hover)', padding: '7px 9px 22px', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, position: 'relative' }}>
              <div style={{ flex: 1, background: 'var(--c-cardBg)', borderRadius: 99, padding: '6px 12px', fontSize: 12, color: 'var(--c-textMuted)', border: '1px solid var(--c-border)' }}>
                Message
              </div>
              <div style={{ width: 28, height: 28, background: 'var(--c-waGreen, #25D366)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', flexShrink: 0, boxShadow: '0 1px 2px rgba(0,0,0,.15)' }}>
    ...
    return (
      <PhoneFrame headerTitle="Your Business" headerSubtitle="online" minHeight={minHeight}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--c-t6, #888)', fontWeight: 500, lineHeight: 1.5, fontFamily: FONT, whiteSpace: 'pre-line' }}>{emptyText}</div>
        </div>
      </PhoneFrame>
    );
  }
  ...
  return (
    <PhoneFrame headerTitle="Your Business" headerSubtitle="online" minHeight={minHeight}>
      ...
      <div style={{ marginLeft: 'auto', maxWidth: '88%', minWidth: '55%' }}>
        <div style={{ background: 'var(--c-sdcf8c6, #DCF8C6)', borderRadius: buttons.length > 0 ? '7.5px 7.5px 0 0' : '7.5px 7.5px 0 7.5px', padding: '6px 7px 5px 9px', boxShadow: '0 1px 0.5px rgba(11,20,26,.13)', position: 'relative', marginRight: buttons.length > 0 ? 0 : 8 }}>
          {buttons.length === 0 && (
          ...
          {template.header_text && template.header_type === 'TEXT' && (
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--c-text)', marginBottom: 4, lineHeight: 1.4, fontFamily: FONT }}>{template.header_text}</div>
          )}
          {template.body && (
            <div style={{ fontSize: 15, color: 'var(--c-text)', lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: FONT }}>{template.body}</div>
          )}
          {template.footer && (
            <div style={{ fontSize: 13, color: 'var(--c-textSecondary)', marginTop: 4, fontFamily: FONT }}>{template.footer}</div>
          )}
          ...
          <div style={{ background: 'var(--c-cardBg)', borderRadius: '0 0 7.5px 7.5px', overflow: 'hidden', boxShadow: '0 1px 0.5px rgba(11,20,26,.13)' }}>
            {buttons.map((btn, i) => (
              <div key={i} style={{ padding: '9px 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, borderTop: '1px solid rgba(0,0,0,.07)' }}>
                <span style={{ fontSize: 15, fontWeight: 500, color: '#00A884', fontFamily: FONT }}>{btn.text || btn.type}</span>
              </div>
            ))}
          </div>
        )}
...
export function BroadcastMessagePreview({ messageType, body, url, mediaLibraryId, caption, mediaItems, minHeight = 280 }) {
        ...
        <div style={{ position: 'relative', width: '100%', borderRadius: 6, overflow: 'hidden', marginBottom: caption ? 6 : 0 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.35)' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,255,255,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M8 5v14l11-7z" fill="var(--c-t1, #111)"/></svg>
            </div>
          </div>
        </div>
      );
    ...
    if (messageType === 'audio') {
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, color: 'var(--c-text)', fontWeight: 500, fontFamily: FONT }}>{selected.name || 'Audio'}</div>
            <div style={{ fontSize: 12, color: 'var(--c-textSecondary)', fontFamily: FONT }}>0:00</div>
          </div>
        </div>
      );
    }
      ...
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0' }}>
          <div style={{ width: 36, height: 36, borderRadius: 8, background: 'var(--c-chatPanel)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z" fill="var(--c-s667781, #667781)"/></svg>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, color: 'var(--c-text)', fontWeight: 500, fontFamily: FONT, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{selected.name || 'Document'}</div>
            <div style={{ fontSize: 12, color: 'var(--c-textSecondary)', fontFamily: FONT }}>{selected.mediaType?.toUpperCase?.() || 'PDF'}</div>
          </div>
        </div>
      );
    }
  ...
  return (
    <PhoneFrame headerTitle="Your Business" headerSubtitle="online" minHeight={minHeight}>
      <div style={{ marginLeft: 'auto', maxWidth: '88%', minWidth: '55%' }}>
        <div style={{ background: 'var(--c-sdcf8c6, #DCF8C6)', borderRadius: '7.5px', padding: '8px 10px', boxShadow: '0 1px 0.5px rgba(11,20,26,.13)' }}>
          {messageType === 'text' && (
            <div style={{ fontSize: 15, color: 'var(--c-text)', lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: FONT }}>
              {body || 'Your message will appear here…'}
            </div>
          )}
          {messageType === 'link' && (
            <div style={{ fontSize: 15, lineHeight: 1.55, fontFamily: FONT }}>
              <a href={/^https?:\/\//i.test(String(url || '')) ? url : '#'} target="_blank" rel="noreferrer" style={{ color: 'var(--c-s027eb5, #027EB5)', textDecoration: 'underline', wordBreak: 'break-all' }} onClick={e => e.preventDefault()}>
          ...
          {['image', 'video', 'audio', 'document'].includes(messageType) && (
            <div>
              {renderMediaPreview()}
              {caption && (
                <div style={{ fontSize: 15, color: 'var(--c-text)', lineHeight: 1.45, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: FONT, marginTop: 4 }}>
                  {caption}
                </div>
              )}
            </div>

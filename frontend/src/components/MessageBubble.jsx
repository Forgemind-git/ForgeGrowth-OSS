...
// Humanize a stored Meta failure (format: "[code] title") into one short,
// actionable line. The webhook handler persists the raw reason onto
// chat_history.error_message; these overrides translate the most common
// delivery failures into plain guidance, falling back to Meta's own title.
const FAILURE_HINTS = {
  131049: "Not delivered — Meta limited marketing messages to this contact (low engagement). Try a Utility template, or wait for them to reply.",
  131047: "Outside the 24-hour window — reach them with an approved template.",
  470:    "Outside the 24-hour window — reach them with an approved template.",
  131026: "Couldn't be delivered — this number can't receive the message (not on WhatsApp, or it blocked you).",
  131051: "This message type isn't supported by WhatsApp.",
  131000: "WhatsApp had a temporary error — try sending again.",
  131008: "Missing a required parameter for this template.",
  132000: "Template parameter mismatch — the message didn't match the approved template.",
};
function friendlyFailure(raw) {
  if (!raw) return 'Message failed to send';
  const m = /^\[(\d+)\]\s*([\s\S]*)$/.exec(raw);
  const code = m ? Number(m[1]) : null;
  const title = m ? m[2] : raw;
  return FAILURE_HINTS[code] || title || 'Message failed to send';
}
...
export default function MessageBubble({ message, isOutgoing, senderAvatarUrl, contactName, onReply, quotedMessage, onReact, canReact, onForward, onStar }) {
  ...
  const MENU_W = 206;
  // Forwardable = anything with text content OR a media file we still hold.
  // Media whose download failed/expired has nothing to re-upload, so it stays
  // out (the bubble already offers a Retry in that state).
  const forwardableMedia = hasMedia
    && mediaStatus !== 'failed'
    && mediaStatus !== 'expired';
  const canForward = !!onForward && (!!message.message_body || forwardableMedia);
    ...
    <>
      ...
      >
        ...
        <div style={bubbleStyle}>
          ...
          {renderContent()}
          {/* Why a send failed — surfaced from Meta's status webhook
              (chat_history.error_message) so a red tick isn't a mystery. */}
          {isOutgoing && message.status === 'failed' && message.error_message && (
            <div style={{
          ...
          {!isVoice && !isTemplate && (
            <div style={{
              ...
              {isOutgoing && (
                <StatusTicks status={message.status} error={message.error_message} />
              )}
            </div>
          )}
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 11, fontSize: 13 }}>
              ...
              {isOutgoing && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, color: C.text, textTransform: 'capitalize' }}>{message.status || 'sent'} <StatusTicks status={message.status} error={message.error_message} /></span>
                </div>
              )}

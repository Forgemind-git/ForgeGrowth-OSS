...
import { RotateCcw, AlertCircle, Mic, Square, Loader2, FileText, Film, Music, ImagePlus, X } from 'lucide-react';
...
// the backend's 1MB JSON limit. Returns { dataUrl, mime, data (base64, no prefix) }.
...
export default function AgentLivePreview({ agentId, headerTitle, canTest = true }) {
  ...
  // Ctrl+V paste an image anywhere in the preview while it's testable.
  useEffect(() => {
    if (!canTest) return;
    const onPaste = (e) => {
